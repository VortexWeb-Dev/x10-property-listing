...
// Prepare data
$file_name     = !empty($property['ufCrm18ReferenceNumber'])
  ? sanitizeFileName($property['ufCrm18ReferenceNumber']) . ".pdf"
  : "Property_$id.pdf";

$title         = $property['ufCrm18TitleEn']       ?? "No Title";
$price         = !empty($property['ufCrm18Price']) ? "AED " . $property['ufCrm18Price'] : "Not Available";
$refId         = $property['ufCrm18ReferenceNumber'] ?? "";
$location      = $property['ufCrm18Location']      ?? "Unknown";
$description   = $property['ufCrm18DescriptionEn'] ?? "No Description";
$images        = $property['ufCrm18PhotoLinks']    ?? [];
$mainImage     = isset($images[0]) ? $images[0] : '';
$size          = $property['ufCrm18Size']          ?? "0";
$bedrooms      = $property['ufCrm18Bedroom']       ?? "0";
$bathrooms     = $property['ufCrm18Bathroom']      ?? "0";
$propertyType  = $property['ufCrm18PropertyType']  ?? "Unknown";
$offeringType = $property['ufCrm18OfferingType'] ?? "Unknown";
$availability  = $property['ufCrm18Availability']  ?? "Unknown";
...
// Agent/Owner info
if ($type === 'agent') {
  $agentName  = $property['ufCrm18AgentName']  ?? "Agent Name";
  $agentEmail = $property['ufCrm18AgentEmail'] ?? "agent@example.com";
  $agentPhone = $property['ufCrm18AgentPhone'] ?? "+97145783581";
} elseif ($type === 'owner') {
  $agentName  = $property['ufCrm18ListingOwner'] ?? "Owner Name";
  // Attempt to fetch owner details from Bitrix
  $userResponse = CRest::call("user.get", [
    "filter" => ["NAME" => $property['ufCrm18ListingOwner']]
  ]);
  $owner       = $userResponse['result'][0] ?? [];
  $agentEmail  = $owner["EMAIL"]          ?? "owner@example.com";
  $agentPhone  = $owner["PERSONAL_MOBILE"] ?? "+97145783581";
} else {
  ...
  $agentPhone = $user['PERSONAL_MOBILE'] ?? "+97145783581";
}

// Amenities
$amenities = $property['ufCrm18PrivateAmenities'] ?? [];
