...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const fetchAndDisplayOptions = async () => {
            try {
                const agentsResponse = await fetch(`${baseUrl}/crm.item.list?entityTypeId=${AGENTS_ENTITY_ID}&select[0]=ufCrm20AgentId&select[1]=ufCrm20AgentName&order[ufCrm20AgentName]=asc`);
                console.log("agent url: ", `${baseUrl}/crm.item.list?entityTypeId=${AGENTS_ENTITY_ID}&select[0]=ufCrm20AgentId&select[1]=ufCrm20AgentName&order[ufCrm20AgentName]=asc`);
                ...
                agents.forEach(agent => {
                    ...
                    option.value = agent.ufCrm20AgentId;
                    option.textContent = agent.ufCrm20AgentName;
