...
<script>
    ...
    async function handleUpdateReference(event) {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}crm.item.update?entityTypeId=${LISTINGS_ENTITY_TYPE_ID}&id=${propertyId}&fields[ufCrm18ReferenceNumber]=${newReference}`);
    ...
    async function getAgent(agentId) {
        const response = await fetch(`${API_BASE_URL}crm.item.list?entityTypeId=${AGENTS_ENTITY_ID}&filter[ufCrm20AgentId]=${agentId}`);
        const data = await response.json();
    ...
    async function handleAction(action, propertyId, platform = null) {
        ...
        switch (action) {
            ...
            case 'duplicate':
                try {
                    ...
                    for (const field in property) {
                        if (
                            field.startsWith('ufCrm18') &&
                            !['ufCrm18ReferenceNumber', 'ufCrm18TitleEn', 'ufCrm18Status', 'ufCrm18PhotoLinks', 'ufCrm18Documents', 'ufCrm18Notes'].includes(field)
                        ) {
                            addUrl += `&fields[${field}]=${encodeURIComponent(property[field])}`;
                        }
                    }

                    if (property['ufCrm18PhotoLinks']) {
                        property['ufCrm18PhotoLinks'].forEach((photoLink, index) => {
                            addUrl += `&fields[ufCrm18PhotoLinks][${index}]=${encodeURIComponent(photoLink)}`;
                        });
                    }

                    if (property['ufCrm18Documents']) {
                        property['ufCrm18Documents'].forEach((document, index) => {
                            addUrl += `&fields[ufCrm18Documents][${index}]=${encodeURIComponent(document)}`;
                        });
                    }

                    if (property['ufCrm18Notes']) {
                        property['ufCrm18Notes'].forEach((note, index) => {
                            addUrl += `&fields[ufCrm18Notes][${index}]=${encodeURIComponent(note)}`;
                        });
                    }

                    addUrl += `&fields[ufCrm18TitleEn]=${encodeURIComponent(property.ufCrm18TitleEn + ' (Duplicate)')}`;
                    addUrl += `&fields[ufCrm18ReferenceNumber]=${encodeURIComponent(property.ufCrm18ReferenceNumber) + '-duplicate'}`;
                    addUrl += `&fields[ufCrm18Status]=DRAFT`;
            ...
            case 'publish':
                apiUrl = `${baseUrl}/crm.item.update?entityTypeId=${LISTINGS_ENTITY_TYPE_ID}&id=${propertyId}&fields[ufCrm18Status]=PUBLISHED`;
                if (platform) {
                    apiUrl += `&fields[ufCrm18${platform.charAt(0).toUpperCase() + platform.slice(1)}Enable]=Y`;
                } else {
                    apiUrl += `&fields[ufCrm18PfEnable]=Y&fields[ufCrm18BayutEnable]=Y&fields[ufCrm18DubizzleEnable]=Y&fields[ufCrm18WebsiteEnable]=Y&fields[ufCrm18Status]=PUBLISHED`;
                }
                break;
            ...
            case 'unpublish':
                ...
                if (platform) {
                    apiUrl += `&fields[ufCrm18${platform.charAt(0).toUpperCase() + platform.slice(1)}Enable]=N`;
                } else {
                    apiUrl += `&fields[ufCrm18PfEnable]=N&fields[ufCrm18BayutEnable]=N&fields[ufCrm18DubizzleEnable]=N&fields[ufCrm18WebsiteEnable]=N&fields[ufCrm18Status]=UNPUBLISHED`;
                }
                break;

            case 'archive':
                if (confirm('Are you sure you want to archive this property?')) {
                    apiUrl = `${baseUrl}/crm.item.update?entityTypeId=${LISTINGS_ENTITY_TYPE_ID}&id=${propertyId}&fields[ufCrm18Status]=ARCHIVED`;
                } else {
                    reloadRequired = false;
                }
                break;
                ...
                if (confirm('Are you sure you want to delete this property?')) {
                    try {
                        ...
                        if (propertyData.result && propertyData.result.item) {
                            ...
                            if (property.ufCrm18PhotoLinks && Array.isArray(property.ufCrm18PhotoLinks)) {
                                console.log('Found photo links:', property.ufCrm18PhotoLinks);
                                for (const imageUrl of property.ufCrm18PhotoLinks) {
                                    try {
                            ...
                            if (property.ufCrm18FloorPlan) {
                                try {
                                    console.log('Attempting to delete floorplan:', property.ufCrm18FloorPlan);
                                    const response = await fetch('./delete-s3-object.php', {
                                        method: 'POST',
                                        headers: {
                                            'Content-Type': 'application/json',
                                        ...
                                        body: JSON.stringify({
                                            fileUrl: property.ufCrm18FloorPlan
                                        })
                                    });
                                    const result = await response.json();
                                    ...
                                    console.error(`Error deleting S3 floorplan: ${property.ufCrm18FloorPlan}`, error);
                                }
                            }

                            // Delete documents from S3
                            if (property.ufCrm18Documents && Array.isArray(property.ufCrm18Documents)) {
                                console.log('Found documents:', property.ufCrm18Documents);
                                for (const docUrl of property.ufCrm18Documents) {
                                    try {
        ...
        if (confirm(`Are you sure you want to ${action} the selected properties?`)) {
            try {
                ...
                const platformFieldMapping = {
                    pf: 'ufCrm18PfEnable',
                    bayut: 'ufCrm18BayutEnable',
                    dubizzle: 'ufCrm18DubizzleEnable',
                    website: 'ufCrm18WebsiteEnable'
                };
                ...
                const requests = propertyIds.map(propertyId => {
                    ...
                    if (action === 'publish') {
                        url += '&fields[ufCrm18Status]=PUBLISHED';

                        if (platformFieldMapping[platform]) {
                            url += `&fields[${platformFieldMapping[platform]}]=Y`;
                        } else {
                            url += `&fields[ufCrm18PfEnable]=Y&fields[ufCrm18BayutEnable]=Y&fields[ufCrm18DubizzleEnable]=Y&fields[ufCrm18WebsiteEnable]=Y`;
                        }
                    } else if (action === 'unpublish') {
                        if (platformFieldMapping[platform]) {
                            url += `&fields[${platformFieldMapping[platform]}]=N`;
                        } else {
                            url += `&fields[ufCrm18PfEnable]=N&fields[ufCrm18BayutEnable]=N&fields[ufCrm18DubizzleEnable]=N&fields[ufCrm18WebsiteEnable]=N&fields[ufCrm18Status]=UNPUBLISHED`;
                        }
                    } else if (action === 'archive') {
                        url += '&fields[ufCrm18Status]=ARCHIVED';
                    }
    ...
    async function fetchProperty(id) {
        ...
        if (data.result && data.result.item) {
            ...
            document.getElementById('available_from').value = formatInputDate(property.ufCrm18AvailableFrom);
            document.getElementById('contract_expiry').value = formatInputDate(property.ufCrm18ContractExpiryDate);

            // Specifications
            document.getElementById('title_deed').value = property.ufCrm18TitleDeed;
            document.getElementById('size').value = property.ufCrm18Size;
            document.getElementById('unit_no').value = property.ufCrm18UnitNo;
            document.getElementById('bathrooms').value = property.ufCrm18Bathroom;
            document.getElementById('parkings').value = property.ufCrm18Parking;
            document.getElementById('total_plot_size').value = property.ufCrm18TotalPlotSize;
            document.getElementById('lot_size').value = property.ufCrm18LotSize;
            document.getElementById('buildup_area').value = property.ufCrm18BuildupArea;
            document.getElementById('layout_type').value = property.ufCrm18LayoutType;
            document.getElementById('project_name').value = property.ufCrm18ProjectName;
            document.getElementById('build_year').value = property.ufCrm18BuildYear;
            Array.from(document.getElementById('property_type').options).forEach(option => {
                if (option.value === property.ufCrm18PropertyType) option.selected = true;
            });
            Array.from(document.getElementById('offering_type').options).forEach(option => {
                if (option.value === property.ufCrm18OfferingType) option.selected = true;
            });
            Array.from(document.getElementById('bedrooms').options).forEach(option => {
                if (option.value == property.ufCrm18Bedroom) option.selected = true;
            });
            Array.from(document.getElementById('furnished').options).forEach(option => {
                if (option.value == property.ufCrm18Furnished) option.selected = true;
            });
            Array.from(document.getElementById('project_status').options).forEach(option => {
                if (option.value == property.ufCrm18ProjectStatus) option.selected = true;
            });
            Array.from(document.getElementById('sale_type').options).forEach(option => {
                if (option.value == property.ufCrm18SaleType) option.selected = true;
            });
            Array.from(document.getElementById('ownership').options).forEach(option => {
                if (option.value == property.ufCrm18Ownership) option.selected = true;
            });

            // Property Permit
            document.getElementById('rera_permit_number').value = property.ufCrm18ReraPermitNumber
            document.getElementById('dtcm_permit_number').value = property.ufCrm18DtcmPermitNumber
            document.getElementById('rera_issue_date').value = formatInputDate(property.ufCrm18ReraPermitIssueDate);
            document.getElementById('rera_expiration_date').value = formatInputDate(property.ufCrm18ReraPermitExpirationDate);

            // Pricing
            document.getElementById('price').value = property.ufCrm18Price;
            document.getElementById('payment_method').value = property.ufCrm18PaymentMethod;
            document.getElementById('downpayment_price').value = property.ufCrm18DownPaymentPrice;
            document.getElementById('service_charge').value = property.ufCrm18ServiceCharge;
            property.ufCrm18HidePrice == "Y" ? document.getElementById('hide_price').checked = true : document.getElementById('hide_price').checked = false;
            Array.from(document.getElementById('rental_period').options).forEach(option => {
                if (option.value == property.ufCrm18RentalPeriod) option.selected = true;
            });
            Array.from(document.getElementById('cheques').options).forEach(option => {
                if (option.value == property.ufCrm18NoOfCheques) option.selected = true;
            });
            Array.from(document.getElementById('financial_status').options).forEach(option => {
                if (option.value == property.ufCrm18FinancialStatus) option.selected = true;
            });

            // Title and Description
            document.getElementById('title_en').value = property.ufCrm18TitleEn;
            document.getElementById('description_en').textContent = property.ufCrm18DescriptionEn;
            document.getElementById('title_ar').value = property.ufCrm18TitleAr;
            document.getElementById('description_ar').textContent = property.ufCrm18DescriptionAr;
            document.getElementById('brochure_description_1').textContent = property.ufCrm18BrochureDescription;
            ...
            // Location
            document.getElementById('pf_location').value = property.ufCrm18Location;
            document.getElementById('pf_city').value = property.ufCrm18City;
            document.getElementById('pf_community').value = property.ufCrm18Community;
            document.getElementById('pf_subcommunity').value = property.ufCrm18SubCommunity;
            document.getElementById('pf_building').value = property.ufCrm18Tower;
            document.getElementById('bayut_location').value = property.ufCrm18BayutLocation;
            document.getElementById('bayut_city').value = property.ufCrm18BayutCity;
            document.getElementById('bayut_community').value = property.ufCrm18BayutCommunity;
            document.getElementById('bayut_subcommunity').value = property.ufCrm18BayutSubCommunity;
            document.getElementById('bayut_building').value = property.ufCrm18BayutTower;

            if (property.ufCrm18Geopoints) {
                const [latitude, longitude] = property.ufCrm18Geopoints.split(',').map(coord => coord.trim());
                document.getElementById('latitude').value = latitude;
                document.getElementById('longitude').value = longitude;
            }

            // Photos and Videos
            document.getElementById('video_tour_url').value = property.ufCrm18VideoTourUrl;
            document.getElementById('360_view_url').value = property.ufCrm_18_360_VIEW_URL;
            document.getElementById('qr_code_url').value = property.ufCrm18QrCodePropertyBooster;
            // Photos
            // Floor Plan

            // Portals
            property.ufCrm18PfEnable == "Y" ? document.getElementById('pf_enable').checked = true : document.getElementById('pf_enable').checked = false;
            property.ufCrm18BayutEnable == "Y" ? document.getElementById('bayut_enable').checked = true : document.getElementById('bayut_enable').checked = false;
            property.ufCrm18DubizzleEnable == "Y" ? document.getElementById('dubizzle_enable').checked = true : document.getElementById('dubizzle_enable').checked = false;
            property.ufCrm18WebsiteEnable == "Y" ? document.getElementById('website_enable').checked = true : document.getElementById('website_enable').checked = false;
            property.ufCrm18Watermark == "Y" ? document.getElementById('watermark').checked = true : document.getElementById('watermark').checked = false;
            ...
            if (property.ufCrm18Notes.length > 0) {
                property.ufCrm18Notes.forEach(note => {
