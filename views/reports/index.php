...
<script>
    window.addEventListener('load', async () => {
        ...
        let properties = [];
        const response = await fetch(`${baseUrl}/crm.item.list?entityTypeId=${LISTINGS_ENTITY_TYPE_ID}&select[0]=ufCrm18OfferingType&select[1]=ufCrm18PfEnable&select[2]=ufCrm18BayutEnable&select[3]=ufCrm18DubizzleEnable&select[4]=ufCrm18WebsiteEnable`);
        const data = await response.json();
        const totalOwners = data.total;

        for (let i = 0; i < Math.ceil(totalOwners / 50); i++) {
            const response = await fetch(`${baseUrl}/crm.item.list?entityTypeId=${LISTINGS_ENTITY_TYPE_ID}&select[0]=ufCrm18OfferingType&select[1]=ufCrm18PfEnable&select[2]=ufCrm18BayutEnable&select[3]=ufCrm18DubizzleEnable&select[4]=ufCrm18WebsiteEnable&start=${i * 50}`);
            const data = await response.json();
        ...
        properties.forEach(property => {
            if (property['ufCrm18OfferingType'] === 'RS' || property['offeringType'] === 'RR') {
                // Residential
                if (property['ufCrm18PfEnable']) residentialPropertyFinder++;
                if (property['ufCrm18BayutEnable']) residentialBayut++;
                if (property['ufCrm18DubizzleEnable']) residentialDubizzle++;
                if (property['ufCrm18WebsiteEnable']) residentialWebsite++;
                if (property['ufCrm18OfferingType'] === 'RS') {
                    residentialSale++;
                } else {
                    residentialRent++;
                }
            } else if (property['ufCrm18OfferingType'] === 'CS' || property['offeringType'] === 'CR') {
                // Commercial
                if (property['ufCrm18PfEnable']) commercialPropertyFinder++;
                if (property['ufCrm18BayutEnable']) commercialBayut++;
                if (property['ufCrm18DubizzleEnable']) commercialDubizzle++;
                if (property['ufCrm18WebsiteEnable']) commercialWebsite++;
                if (property['ufCrm18OfferingType'] === 'CS') {
                    commercialSale++;
                } else {
                    commercialRent++;
                }
