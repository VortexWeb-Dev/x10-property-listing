...
<script>
    ...
    async function fetchAgents(page = 1) {
        ...
        const apiUrl = `${baseUrl}/crm.item.list?entityTypeId=${entityTypeId}&order[id]=desc&select[0]=id&select[1]=ufCrm20AgentName&select[2]=ufCrm20AgentEmail&select[3]=ufCrm20AgentMobile&select[4]=ufCrm20AgentLicense&select[5]=ufCrm20AgentPhoto&select[6]=ufCrm20AgentId&start=${(page - 1) * pageSize}`;
        ...
        try {
            ...
            agentList.innerHTML = agents
                .map(
                    (agent) => `
                <tr>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">${agent.ufCrm20AgentId}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 flex items-center space-x-2">
                        <img 
                            class="w-10 h-10 object-cover rounded-full" 
                            src="${agent.ufCrm20AgentPhoto || './assets/images/agent-placeholder.webp'}" 
                            alt="${agent.ufCrm20AgentName || 'Agent'}" />
                        <span>${agent.ufCrm20AgentName || 'Agent'}</span>
                    </td>

                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">${agent.ufCrm20AgentEmail || ''}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">${agent.ufCrm20AgentMobile || ''}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">${agent.ufCrm20AgentLicense || ''}</td>
