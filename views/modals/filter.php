...
<script>
    function prepareFilters(e) {
        ...
        const fieldMappings = {
            'city': '%ufCrm18City',
            'community': '%ufCrm18Community',
            'subCommunity': '%ufCrm18SubCommunity',
            'building': '%ufCrm18Tower',
            'reference': 'ufCrm18ReferenceNumber',
            'permit': 'ufCrm18ReraPermitNumber',
            'listing_title': '%ufCrm18TitleEn',
            'property_type': 'ufCrm18PropertyType',
            'saleRent': 'ufCrm18OfferingType',
            'listing_agent': '%ufCrm18AgentName',
            'developer': '%ufCrm18Developers',
            'listing_owner': '%ufCrm18ListingOwner',
            'landlordEmail': '%ufCrm18LandlordEmail',
            'landlordPhone': '%ufCrm18LandlordContact',
            'bedrooms': 'ufCrm18Bedroom',
            'bathrooms': 'ufCrm18Bathroom',
            'price': 'ufCrm18Price',
            'portal': 'portal'
        }
        ...
        if (filterParams['portal']) {
            if (filterParams['portal'] == 'PF') {
                filterParams['ufCrm18PfEnable'] = 1
            } else if (filterParams['portal'] == 'BAYUT') {
                filterParams['ufCrm18BayutEnable'] = 1
            } else if (filterParams['portal'] == 'DUBIZZLE') {
                filterParams['ufCrm18DubizzleEnable'] = 1
            } else if (filterParams['portal'] == 'WEBSITE') {
                filterParams['ufCrm18WebsiteEnable'] = 1
            }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const fetchAgents = async () => {
            try {
                const response = await fetch(`${baseUrl}/crm.item.list?entityTypeId=${AGENTS_ENTITY_ID}&select[0]=id&select[1]=ufCrm20AgentName&order[ufCrm20AgentName]=asc`);
        ...
        const fetchDevelopers = async () => {
            try {
                const response = await fetch(`${baseUrl}/crm.item.list?entityTypeId=${DEVELOPERS_ENTITY_ID}&select[0]=id&select[1]=ufCrm26DeveloperName&order[ufCrm26DeveloperName]=asc`);
                const data = await response.json();
                const totalDevelopers = data.total;
                const developers = [];

                for (let i = 0; i < Math.ceil(totalDevelopers / 50); i++) {
                    const paginatedResponse = await fetch(`${baseUrl}/crm.item.list?entityTypeId=${DEVELOPERS_ENTITY_ID}&select[0]=id&select[1]=ufCrm26DeveloperName&order[ufCrm26DeveloperName]=asc&start=${i * 50}`);
        ...
        const fetchAndDisplayOptions = async () => {
            try {
                ...
                createSelectOptions(agents, listingAgentSelect, 'ufCrm20AgentName', 'ufCrm20AgentName');
                createSelectOptions(developers, developerSelect, 'ufCrm26DeveloperName', 'ufCrm26DeveloperName');
